...
The majority of accidents occur in July with the fewest in February. Surprisingly, there is not as commensorate a drop in the number of fatalities in February as compared with July as one may expect. Although lower during February, the number of fatalities remains fairly constant across the year at around 55,000 deaths. There is a peak of suspected major and minor injuries in July, which is not reflected among the confirmed injuries. Fatal injuries, for instance, are elevated all summer. The number of accidents that do not result in any injury remains fairly steady across all months around 31,000 incidents. 
...
### Graph 18: Analyzing the Relationship Between Time and Accidents Across Different Months 
...
## Graph 3: Distribution of Age of Persons Involved in Accidents {.tabset}

The majority of accidents occur at night and the fewest occur during the evening. Starting in the evening, there are fewer people over the age of 60 getting in accidents which is likely a result of that group not being on the road as much as young and middle aged people.
...
### Late night
```{r Distribution of Age of Persons Involved in Accidents Evening Late Night, message=FALSE}
SexAgeGraph <- Master_Dataframe %>% 
...
We can see that the majority of accidents occur late at night which is followed by the mid-day and nighttime periods. 
...
## Graph 11: Relationship between Hour of Day and Age of Persons Involved in Accidents, Colored by Injury Severity and Faceted by Sex 
...
### Graph 19: Time-based analysis - Seasonal Analysis
...
DF_For_Shapiro <- Master_Dataframe %>% 
  slice_sample(n = 5000, replace = FALSE)# Shapiro-Wilk test has a maximum limit of 5,000 observations

```

The number of accidents by hour does not follow a normal distribution when starting at midnight, but shifting the data by six hours makes the data follow a bell curve much more closely. This is reflected both in the histogram and in the QQ plot which is initially curved then aligns almost perfectly with the 1 to 1 ratio once shifted. This suggests that there is a significant different in the patterns of accidents occuring at night vs during the day. We can attribute this to factors such as reduced visibility and fatigue.

``` {r Distribution Hour}
# Shifted variable
Accident_hour_hist <- ggplot(Master_Dataframe, aes(x = Accident_hour)) +
    geom_histogram(binwidth = 1)
Accident_hour_shifted <- ggplot(Master_Dataframe, aes(x = Accident_hour_shifted)) +
    geom_histogram(binwidth = 1) +
...
Accident_hour_shiftedQQ <- ggplot(data.frame(Master_Dataframe), aes(sample = Accident_hour_shifted)) +
  geom_qq() +
  geom_qq_line(color = "indianred3") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles", title = "QQ Plot for Accident Hour")

Accident_hour_QQ <- ggplot(data.frame(Master_Dataframe), aes(sample = Accident_hour)) +
  geom_qq() +
  geom_qq_line(color = "indianred3") +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles", title = "QQ Plot for Accident Hour")
