...
### Graph 4: Heatmap of Accidents by Hour and Day of the Week
...
## Graph 5: Distribution of Age of Persons Involved in Accidents {.tabset}
...
## Accidents Caused By Intoxication And Gender Over The Day {.tabset}
...
## Graph 6: Relationship between Hour of Day and Age of Persons Involved in Accidents, Colored by Injury Severity and Faceted by Sex 
...
## Graph 7: Distribution of Age by Sex Involved in Accidents
...
```
Although an improvement over the first order polynomial model, there remains a slight curve that has critical points around 2005 and 2011 with a quadratic regression fit. This indicates that the trend is not linear and instead follows a polynomial trajectory.  
...
```
The non-parametric fit is a significant improvement compared to the linear or polynomial fits (as the loess is almost a straight line), suggesting the number of accidents has some non-linear and/or non-monotonic components. 
...
```
We can see that the plot follows a yearly oscillation cycle which peaks during the summer time and is the lowest during the winter. Since driving conditions are highly regional, it is difficult to glean to what extent the model is accurately accounting for changes in driving conditions or habits. Therefore, it is important to understand the regional factors before moving forward.

#### Regional Analysis of first order residuals
``` {r Added to explore seasonality further, fig.width = 12}
...
``` {r Exploring seasonality 3, fig.width = 12}
...
``` {r Exploring seasonality 4, fig.width = 12}
...
```
The trends do not appear to be monotonic (except for May, October, and to a lesser extent September) which suggests that the cyclical nature of accidents occurring is not systemic. 
...
There are clear patterns and trends in car accidents that can be identified based on the time of year and time of the day which can be made more specific using factors such as age or gender. The majority of accidents occur in July, with the fewest in February, following a cyclic pattern. Additionally, New Years and July 4th are very dangerous times to be on the road. Across the entire day, accidents tend to involve younger individuals, whereas fatal accidents tend to involve older individuals than non-fatal accidents.
