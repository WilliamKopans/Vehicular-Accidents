...
knitr::opts_chunk$set(echo = TRUE,
                      ...
                      cache.rebuild = FALSE,
                      cache=TRUE, 
                      cache.extra=list(png="images/", pdf="documents/"))
...
ggplot(Names_Changed, aes(x = Accident_hour, y = Age, color = Injury_Severity_Basic)) +
...
Plot 31: Bivariate relationships - Scatter plot
...
Plot 32: Time-based analysis - Seasonal decomposition plot
...
Plot 33: Number of vehicles and people
...
Plot 35: Age vs Number of Persons Involved by Sex
...
Plot 36: Violin plot of Age by Sex
...
Plot 37: Hexbin plot of Age vs Number of Persons Involved
...
Plot 38: Ridge plot of Age distribution by Injury Severity (Why are these different?)
...
Plot 39: Bar plot of Number of Accidents by Vehicle Body Type, faceted by Alcohol Involvement (Double check `accidents_by_body_type_injury_sex` to make sure this is working properly)
...
Plot 40: Scatter plot with jitter for the Number of Persons Involved in Accidents by Hour, colored by Sex
...
Plot 41: Bubble plot for the number of accidents by state and injury severity
...
Plot 42: Bar plot for the number of accidents by month and year
...
Plot 43: Ridgeline plot of the number of accidents by state and hour. RI and DC have some issues with early morning accidents!
``` {R Plot 43, fig.cap="Ridgeline plot of number of accidents by state and hour"}
ggplot(Names_Changed, aes(x = Accident_hour, y = state_name, fill = state_name)) +
    geom_density_ridges() +
    labs(title = "Ridgeline Plot: Accident Hour", x = "Hour", y = "State") +
    theme_ridges()
```

Plot 44: Boxplot of Age by Injury Severity
``` {r Plot 44, fig.cap="Boxplot of Age by Injury Severity"}
...
Plot 45: Normality: Q-Q plot of Age
``` {r Plot 45, Normality: Q-Q plot of Age}
...
Plot 46: Linearity: Scatterplot of Age vs. Number_of_persons_involved with a LOESS fit (takes forever to render)
``` {r Plot 46, Linearity: Scatterplot of Age vs. Number_of_persons_involved with a LOESS fit}
# ggplot(Names_Changed, aes(x = Age, y = Number_of_persons_involved)) +
...
Plot 47: Homoscedasticity: Residual plot (using a linear model as an example) (takes forever to render)
``` {r Plot 47, Residual plot}
# # Fit a linear model
...
Plot 48: Density plot of Age (to check the distribution)
``` {r Plot 48, Density plot of Age}
ggplot(Names_Changed, aes(x = Age)) +
