...
# https://www.cdc.gov/nchs/hus/sources-definitions/geographic-region.htm#:~:text=Geographic%20region&text=Northeast%3A%20Includes%20Connecticut%2C%20Maine%2C,Ohio%2C%20South%20Dakota%2C%20and%20Wisconsin
...
set.seed(123)
master_df <- bind_rows(
  south %>% mutate(region = "South") %>% slice_sample(n = 25000, replace = FALSE),
  west %>% mutate(region = "West") %>% slice_sample(n = 25000, replace = FALSE),
  northeast %>% mutate(region = "Northeast") %>% slice_sample(n = 25000, replace = FALSE),
  midwest %>% mutate(region = "Midwest") %>% slice_sample(n = 25000, replace = FALSE) 
)

ggplot(data = master_df, aes(x=Datetime,y=rows_on_date, col = region))+
  # geom_point(alpha = 1/4) +
  labs(
    title = "Mean Accidents (loess) Per Day in 60 Day Window With 15 Day Overlap",
    x = "Date",
    y = "Mean Number of Accidents Per Day (Log Base 10)",
...
### Graph 35: Age vs Number of Persons Involved by Sex - Make hex plot?
...
### Graph 38: Ridge plot of Age distribution by Injury Severity (Why are these different?) - Make density plots?
...
### Graph 40: Scatter plot with jitter for the Number of Persons Involved in Accidents by Hour, colored by Sex - Can this be a hex plot?
...
# Test if reigions like New England where hours of daylight will change and may cause more accidents 
